...
界面本身分为两半，每一半都与其对应的细节无关。 首先是 struct clk 的通用定义，它统一了传统上跨各种平台复制的框架级会计和基础设施。 第二个是 clk.h api 的通用实现，在drivers/clk/clk.c 中定义。 最后是 struct clk_ops，它的操作由 clk api 实现调用。 
