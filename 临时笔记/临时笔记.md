...
&emsp;&emsp;此手册侧重于使新的 thermal zone 和冷却设备能够参与 thermal 管理。
...
```
&emsp;&emsp;此接口函数将一个新的 thermal zone 设备（传感器）添加到 /sys/class/thermal 路径中作为 thermal_zone[0-*]。 同时，它会尝试绑定所有注册的热冷却设备。 
   * type：thermal zone 类型
   * trips：此 thermal zone 支持的触发点总数
   * mask：Bit string：如果设置了第‘n’位，则触发点‘n’是可写的。
   * devdata：设备私有数据
   * ops：thermal zone 设备回调函数
     * bind：将 thermal zone 设备和冷却设备绑定。
     * unbind：将 thermal zone 设备和冷却设备解绑。
     * get_temp：获取当前 thermal zone 的温度。
     * set_trips：设置触发点窗口。每当更新当前温度时，都会快速找到当前温度之下和之上的触发点。
...
#### 1.1 通用信息
&emsp;&emsp;CPU 频率代码位于`drivers/cpufreq/cpufreq.c`。此文件中的代码为基于 cpu 频率架构的驱动（实际中执行频率转换的代码段）提供了标准接口和“通知程序”。这些驱动是指：设备驱动程序，内核中一部分需要被通知更改策略的驱动（例如 ACPI thermal 模块），更改所有频率的驱动（例如 timing code），甚至是需要强制控制速度的驱动（例如 ARM 架构上的 LCD 驱动）。此外，内核“常量”loops_per_jiffy 会根据此处的频率变化进行更新。
&emsp;&emsp;cpufreq 策略的引用计数由`cpufreq_cpu_get`和`cpufreq_cpu_put`完成，确保 cpufreq 驱动程序正确注册到核心，并且在调用`cpufreq_cpu_put`之前不会卸载。 这也确保了相应的 cpufreq 策略在使用时不会被释放。 
#### 1.2 cpufreq 通知程序
&emsp;&emsp;CPUFreq 通知程序符合标准内核通知程序接口。 有关通知程序的详细信息，请参见 linux/include/linux/notifier.h。
&emsp;&emsp;有两种不同的 CPUFreq 通知程序：策略通知程序和转换通知程序。 
##### 1.2.1 CPUFreq policy notifiers
&emsp;&emsp;These are notified when a new policy is intended to be set.一个策略转换会调用<u> cpu 频率策略通知程序</u>两次。
1. During CPUFREQ_ADJUST，所有的 cpu 频率通知程序可能会更改频率限制：如果它们看到了这样做的需要————可能出于 thermal 考虑或者是硬件的限制。
2. During CPUFREQ_NOTIFY，所有的 notifiers 被告知新的策略————如果两个硬件驱动程序在此阶段之前未能就新策略达成一致，则不兼容的硬件将被关闭，用户也将被告知。

&emsp;&emsp;phase 在通知程序的第二个参数中指定。
&emsp;&emsp;第三个参数，一个`void *pointer`指针，指向一个`struct cpufreq_policy`结构体，此结构体包含了几个值：min, max (the lower and upper frequencies (in kHz) of the new policy)。

##### 1.2.2 CPUFreq transition notifiers
&emsp;&emsp;当 CPUfreq 驱动程序切换 CPU 核心频率并且此更改没有任何外部影响时，策略中的每个在线 CPU 都会被通知两次。
&emsp;&emsp;第二个参数指定了 phase：CPUFREQ_PRECHANGE 或 CPUFREQ_POSTCHANGE。
&emsp;&emsp;第三个参数是一个`struct cpufreq_freqs`结构体，它包含了下面的值：
* cpu: 影响到的 cpu 的数量
* old: 之前的频率
* new: 新的频率
* flags: cpufreq 驱动的标志位

##### 1.2.3 CPUFreq Table Generation with Operating Performance Point (OPP)
&emsp;&emsp;关于 OPP 的细节，参看文档`Documentation/power/opp.txt`
&emsp;&emsp;`dev_pm_opp_init_cpufreq_table`：此函数提供了一个随时可用的例程来将 OPP 层内部可用的频率信息转换为可以提供给 cpufreq 的格式。
&emsp;&emsp;警告：不要在中断上下文使用此函数。
&emsp;&emsp;示例：
```c
soc_pm_init()
{
...
```
&emsp;&emsp;注意：此功能仅在启用 CONFIG_CPU_FREQ 和 CONFIG_PM_OPP 时可用。 
&emsp;&emsp;`dev_pm_opp_free_cpufreq_table`：释放 dev_pm_opp_init_cpufreq_table 分配的表。
### 2 cpufreq-stats.txt
#### 2.1 介绍
&emsp;&emsp;cpufreq-stats 是一个为每个 CPU 提供 CPU 频率统计信息的驱动程序。这些统计信息在`/sysfs`目录下作为一串只读接口出现。对于每个 CPU，此接口（配置后）将出现在 /sysfs (\<sysfs root>/devices/system/cpu/cpuX/cpufreq/stats/) 中 cpufreq 下的单独目录中。 各种统计信息会在该目录下形成只读文件。 
&emsp;&emsp;此驱动程序被设计为独立于<u>运行在任何 cpu 上的任何特定的 cpufreq 驱动</u>。
#### 2.2 提供的统计数据（示例）
&emsp;&emsp;cpufreq stats 提供以下统计数据（详细解释如下）。 
* time_in_state
* total_trans
* trans_table
  
&emsp;&emsp;所有统计数据都来自从插入统计数据驱动程序的时刻（或重置统计数据的时间）到读取特定统计数据的时刻。显然，在插入 stats 驱动程序之前不会有任何关于频率转换的信息。
```shell
...
```
* reset：这是一个只读属性，此属性可用来复位计数器的状态。这对于评估不同调控器下的系统行为非常有用，不需要重新启动。 
* time_in_state：这个属性给出了<u>在此 CPU 支持的每个频率上花费的时间量</u>。cat 输出的每一行都会有`<frequency> <time>`对，这意味着这个 CPU 在`<frequency>`上花费了`<time>`个用户时间单位的时间。CPU 支持的每一个频率都会有一行输出。此处的用户时间单位是 10 毫秒（类似于 /proc 中导出的其他时间）。 
示例：
```shell
...
```
* total_trans
&emsp;&emsp;这给出了该 CPU 上的频率转换总数。cat 输出将有一个计数，即频率转换的总数。 
示例：
```shell
<mysystem>:/sys/devices/system/cpu/cpu0/cpufreq/stats # cat total_trans
20
```
* trans_table：这将提供有关所有 CPU 频率转换的细粒度信息。 这里的 cat 输出是一个二维矩阵，其中条目 <i,j>（第 i 行，第 j 列）表示从 Freq_i 到 Freq_j 的转换次数的计数。 Freq_i 是随着行数的增加而降序排列，而 Freq_j 是随着列数的增加而降序排列。 此处的输出还包含每行和每列的实际频率值，以提高可读性。
如果转换表大于 PAGE_SIZE，读取此值将返回 -EFBIG 错误。 
示例：
```shell
...
#### 2.3 配置 cpufreq-stats
&emsp;&emsp;在 kernel 中配置 cpufreq-stats
```shell
Config Main Menu
...
```
&emsp;&emsp;使能`CPU Frequency scaling (CONFIG_CPU_FREQ)`来配置 cpufreq-stats。
&emsp;&emsp;`CPU frequency translation statistics (CONFIG_CPU_FREQ_STAT)`提供包括time_in_state、total_trans 和trans_table 的统计信息。 
&emsp;&emsp;一旦启用此选项并且您的 CPU 支持 cpufrequency，您将能够在 /sysfs 中看到 CPU 频率统计信息。 

### 3 cpu-drivers.txt
#### 3.1 What To Do?
&emsp;&emsp;你刚拿到一个全新的带有数据手册的芯片或芯片组，并且想为该芯片或芯片组添加 cpufreq 该怎么办？很好，下面是一些关于什么是必要的一些提示。
##### 3.1.1 初始化
&emsp;&emsp;首先，在 __initcall 级别 7 (module_init()) 或更高级别的函数中检查该内核是否在正确的 CPU 和正确的芯片组上运行。 如果是这样，请使用 `cpufreq_register_driver()` 向 CPUfreq 内核注册 `struct cpufreq_driver`。
&emsp;&emsp;此结构体`cpufreq_driver`应该包含什么。
* `.name`：驱动的名字
* `.init`：指向每个策略初始化函数的指针。
* `.verify`：指向“verification”函数的指针。
* `.setpolicy`：`.fast_switch`或`.target`或`.target_index`——不同点看后文。

可选项
* `.flags`：cpufreq 内核的提示。 
* `.driver_data`：cpufreq 驱动程序特定数据。 
* `.resolve_freq`：返回最适合目标频率的频率。尽管不改变频率。 
* `.get_intermediate`和`.target_intermediate`：用于在改变 CPU 频率时切换到稳定频率。 
* `.get`：返回 CPU 的当前频率。
* `.bios_limit`：返回 CPU 的硬件/BIOS 最大频率限制。 
* `.exit`：指向在 CPU 热插拔进程的 CPU_POST_DEAD 阶段调用的按策略清理函数的指针。 
* `.stop_cpu`：指向在 CPU 热插拔进程的 CPU_DOWN_PREPARE 阶段调用的按策略停止函数的指针。 
* `.suspend`：一个指向每种策略悬挂函数的指针，which is called with interrupts disabled and _after_ the governor is stopped for the policy.
* `.resume`：一个指向每种策略恢复函数的指针，which is called with interrupts disabled and _before_ the governor is started again.
* `.ready`：一个指向每种策略就绪函数的指针，该函数在策略完全初始化后调用。
* `.attr`：指向以 NULL-terminated 的“struct freq_attr”列表的指针，该列表允许将值导出到sysfs。
* `.boost_enabled`：如果设置，则启用提升频率（boost frequencies）。 
* `.set_boost`：指向每个策略函数的指针，用于启用/禁用提升频率。 

##### 3.1.2 每个 cpu 初始化
&emsp;&emsp;每当向设备模型注册新 CPU 时，或在 cpufreq 驱动程序注册自身后，如果 CPU 不存在 cpufreq 策略，则调用每个策略初始化函数`cpufreq_driver.init`。请注意，`.init()`和`.exit()`例程只被策略调用一次，而不是被管理策略的每个 CPU 调用。它使用结构体`cpufreq_policy *policy`作为参数。现在要做什么？
&emsp;&emsp;如有必要，请激活 CPU 上的 CPUfreq 支持。 
&emsp;&emsp;接下来，驱动必须填写以下值：
* `policy->cpuinfo.min_freq`和`policy->cpuinfo.max_freq`：CPU 所支持的最大和最小频率（以 KHz 为单位）。
* `policy->cpuinfo.transition_latency`：此 CPU 在两个频率之间切换所需的时间（以纳秒为单位）（如果适用，则指定 CPUFREQ_ETERNAL）。
* `policy->cur`：此 CPU 的当前工作频率（如果适用）。
* `policy->min`、`policy->max`、`policy->policy`如果有必要`policy->governor`：必须包含此 CPU 的“默认策略”。A few moments later, 使用这些值来调用`cpufreq_driver.verify`和`cpufreq_driver.setpolicy`或`cpufreq_driver.target/target_index`。
* `policy->cpus`：Update this with the masks of the (online + offline) CPUs that do DVFS along with this CPU (i.e.  that share clock/voltage rails with it). 使用与此 CPU 一起执行 DVFS 的（在线 + 离线）CPU 的掩码更新它（即与其共享时钟/电压轨）。 

&emsp;&emsp;对于设置其中一些值（cpuinfo.min[max]_freq，policy->min[max]），频率表助手可能会有所帮助。 有关它们的更多信息，请参阅第 2 节。 

##### 3.1.2 verify 核实
&emsp;&emsp;当用户决定设置新策略（由“policy,governor,min,max”组成）时，必须验证该策略，以便更正不兼容的值。 为了验证这些值 cpufreq_verify_within_limits(struct cpufreq_policy *policy, unsigned int min_freq, unsigned int max_freq) 函数可能会有所帮助。 有关频率表助手的详细信息，请参阅第 2 节。 
&emsp;&emsp;您需要确保至少一个有效频率（或操作范围）在 policy->min 和 policy->max 之内。 如有必要，首先增加policy->max，只有当这不是解决方案时，才减少policy->min。 
##### 3.1.4 target 或 target_index 或 setpolicy 或 fast_switch 
&emsp;&emsp;大多数 cpufreq 驱动程序甚至大多数 cpu 频率缩放算法只允许将 CPU 频率设置为预定义的固定值。鉴于此，您使用 ->target()、->target_index() 或 ->fast_switch() 回调。 
&emsp;&emsp;一些支持 cpufreq 的处理器会自行在特定限制之间切换频率。 这些应使用 ->setpolicy() 回调。 
##### 3.1.5 target/target_index
&emsp;&emsp;target_index 调用有两个参数：struct cpufreq_policy *policy 和 unsigned int index（into the exposed frequency table）。
&emsp;&emsp;CPUfreq 驱动程序在此处调用时必须设置新频率。 实际频率必须由freq_table[index].frequency 确定。
&emsp;&emsp;如果出现错误，它应该始终恢复到较早的频率（即 policy->restore_freq），即使我们更早地切换到中频。 

###### 已弃用：
&emsp;&emsp;目标调用有三个参数：struct cpufreq_policy *policy,
unsigned int target_frequency, unsigned int relation.
&emsp;&emsp;CPUfreq 驱动程序在此处调用时必须设置新频率。 必须使用以下规则确定实际频率： 
* 尽可能接近“target_freq”
* policy->min <= new_freq <= policy->max (这必须是有效的!!!)
* 如果relation==CPUFREQ_REL_L，尝试选择一个高于或等于target_freq的new_freq。 （“L 表示最低，但不低于”）
* 如果relation==CPUFREQ_REL_H，尝试选择一个低于或等于target_freq的new_freq。 （“H 表示最高，但不高于”） 

&emsp;&emsp;这里频率表助手可能会再次帮助您，有关详细信息，请参阅第 2 节。 

##### 3.1.6 fast_switch
&emsp;&emsp;此函数用于从调度程序上下文进行频率切换。 并非所有驱动程序都应该实现它，因为不允许在此回调中休眠。 此回调必须高度优化以尽快进行切换。
&emsp;&emsp;这个函数有两个参数：struct cpufreq_policy *policy 和 unsigned int target_frequency。 
##### 3.1.7 setpolicy
setpolicy 调用仅采用 struct cpufreq_policy *policy 作为参数。You need to set the lower limit of the in-processor or in-chipset dynamic frequency switching to policy->min, the upper limit to policy->max, and -if supported- select a performance-oriented setting when policy->policy is CPUFREQ_POLICY_PERFORMANCE, and a powersaving-oriented setting when CPUFREQ_POLICY_POWERSAVE. Also check the reference implementation in drivers/cpufreq/longrun.c
##### 3.1.8 get_intermediate 和 target_intermediate 
&emsp;&emsp;仅适用于未设置 target_index() 和 CPUFREQ_ASYNC_NOTIFICATION 的驱动程序。 
&emsp;&emsp;get_intermediate 应该返回一个平台想要切换到的稳定的频率，并且 target_intermediate() 应该在跳转到“index”对应的频率之前将 CPU 设置为该频率。 Core 将负责发送通知，驱动程序不必在 target_intermediate() 或 target_index() 中处理它们。 
&emsp;&emsp;驱动程序可以从 get_intermediate() 返回“0”，以防他们不希望为某个目标频率切换到中间的频率。在这种情况下，核心将直接调用 ->target_index()。 
&emsp;&emsp;注意：->target_index() 应该恢复到 policy->restore_freq 以防出现故障，因为核心会为此发送通知。 
#### 3.1 Frequency Table Helpers
&emsp;&emsp;由于大多数 cpufreq 处理器仅允许设置为几个特定频率，因此具有某些功能的“频率表”可能有助于处理器驱动程序的某些工作。 这样的“频率表”由一个 struct cpufreq_frequency_table 条目数组组成，在“driver_data”中有特定于驱动程序的值，在“frequency”中对应的频率和设置的标志。 在表的末尾，您需要添加一个cpufreq_frequency_table 条目，频率设置为CPUFREQ_TABLE_END。 如果您想跳过表中的一项，请将频率设置为 CPUFREQ_ENTRY_INVALID。 条目不需要按任何特定顺序排序，但如果它们是 cpufreq 核心，则会为它们快速执行 DVFS，因为搜索最佳匹配会更快。 
&emsp;&emsp;如果策略在其 policy->freq_table 字段中包含有效指针，则核心会自动验证 cpufreq 表。 
&emsp;&emsp;cpufreq_frequency_table_verify() 确保至少一个有效频率在 policy->min 和 policy->max 内，并且满足所有其他标准。 这对 ->verify 调用很有帮助。 
&emsp;&emsp;cpufreq_frequency_table_target() 是 ->target 阶段对应的频率表助手。 只需将值传递给此函数，此函数将返回频率表条目的 ，其中包含 CPU 应设置的频率。 
&emsp;&emsp;以下宏可用作 cpufreq_frequency_table 上的迭代器：
* cpufreq_for_each_entry(pos, table)：迭代频率表的所有条目。 
* cpufreq_for_each_valid_entry(pos, table)：迭代所有条目，不包括 CPUFREQ_ENTRY_INVALID 频率。 
...
```
&emsp;&emsp;如果您需要处理 pos 在 driver_freq_table 中的位置，请不要减去指针，因为它非常耗费资源。 相反，使用宏 cpufreq_for_each_entry_idx() 和 cpufreq_for_each_valid_entry_idx()。 


### 4 opp.txt
#### 4.1 介绍
##### 4.1.1 什么是 Operating Performance Point (OPP)
&emsp;&emsp;今天的复杂 SoC 由多个协同工作的子模块组成。 在执行各种用例的操作系统中，并非 SoC 中的所有模块都需要始终以其最高性能频率运行。 为促进这一点，SoC 中的子模块被分组到域中，允许某些域以较低的电压和频率运行，而其他域则以较高的电压/频率对运行。 
&emsp;&emsp;由设备将支持的每个域的频率和电压对组成的一组离散元组称为 Operating Performance Points 或 OPP。 
&emsp;&emsp;示例：
&emsp;&emsp;一个 MPU 设备支持下面参数
```shell
{300MHz at minimum voltage of 1V}, 
{800MHz at minimum voltage of 1.2V},
{1GHz at minimum voltage of 1.3V}
```
我们可以将上面的参数表示为下面三对 OPP {Hz, uV} 数组：
```shell
{300000000, 1000000}
{800000000, 1200000}
{1000000000, 1300000}
```
##### 4.1.2 OPP 库
&emsp;&emsp;OPP 库提供了一组帮助函数来组织和查询 OPP 信息。 库位于drivers/base/power/opp.c，头文件位于include/linux/pm_opp.h。 可以通过从电源管理菜单配置菜单启用 CONFIG_PM_OPP 来启用 OPP 库。 OPP 库依赖于 CONFIG_PM，因为某些 SoC（例如 Texas Instrument 的 OMAP 框架）允许在不需要 cpufreq 的情况下选择性地在某个 OPP 启动。 
&emsp;&emsp;OPP 库的典型使用如下所示：
```shell
(users)		-> registers a set of default OPPs		-> (library)
SoC framework	-> modifies on required cases certain OPPs	-> OPP layer
		-> queries to search/retrieve information	->
```
&emsp;&emsp;OPP 层期望每个域由唯一的设备指针表示。 SoC 框架向 OPP 层注册每个设备的一组初始 OPP。 预计此列表将是一个最佳的小数字，通常每个设备大约 5 个。 此初始列表包含框架希望在系统中默认安全启用的一组 OPP。

###### OPP 可用性注意事项：
&emsp;&emsp;随着系统继续运行，SoC 框架可能会根据各种外部因素选择使某些 OPP 在每个设备上可用或不可用。 示例用法：热管理或其他特殊情况，其中 SoC 框架可能会选择禁用更高频率的 OPP 以安全地继续操作，直到可以重新启用该 OPP（如果可能）。 

&emsp;&emsp;OPP 库在它的实现中促进了这个概念。 以下操作功能仅在可用的 ops 上运行：
```c
opp_find_freq_{ceil, floor}, 
dev_pm_opp_get_voltage, 
dev_pm_opp_get_freq, 
dev_pm_opp_get_opp_count
```
&emsp;&emsp;dev_pm_opp_find_freq_exact 旨在用于查找 opp 指针，然后可将其用于 dev_pm_opp_enable/disable 函数以根据需要使 opp 可用。 
&emsp;&emsp;警告：如果为设备调用了 dev_pm_opp_enable/disable 函数，OPP 库的用户应使用 get_opp_count 刷新其可用性计数，触发这些函数的确切机制或对其他相关子系统（如 cpufreq）的通知机制由 SoC 特定的决定使用 OPP 库的框架。 在这些操作的情况下，需要注意刷新 cpufreq 表。

#### 4.2 初始 OPP 列表注册 
&emsp;&emsp;SoC 实现迭代调用 dev_pm_opp_add 函数来为每个设备添加 OPP。 预计 SoC 框架将注册 OPP 条目最佳 - 典型数量范围小于 5。通过注册 OPP 生成的列表由 OPP 库在整个设备运行过程中维护。 SoC 框架随后可以使用 dev_pm_opp_enable/disable 函数动态控制 OPP 的可用性。 
&emsp;&emsp;dev_pm_opp_add：为设备指针表示的特定域添加新的 OPP。 OPP 是使用频率和电压定义的。 添加后，假定 OPP 可用，并且可以使用 dev_pm_opp_enable/disable 函数控制其可用性。 OPP 库内部在 opp 结构中存储和管理此信息。 SoC 框架可以使用此功能来根据 SoC 使用环境的需求定义最佳列表。 
&emsp;&emsp;警告：不要在中断上下文中使用此函数。 
&emsp;&emsp;示例：
```c
soc_pm_init()
{
...
```
#### 4.3 OPP search 函数
&emsp;&emsp;cpufreq 等高级框架在频率上运行。 为了将频率映射回相应的 OPP，OPP 库提供了方便的功能来搜索 OPP 库内部管理的 OPP 列表。 如果找到匹配项，这些搜索函数返回表示 opp 的匹配指针，否则返回错误。 这些错误应该由标准错误检查（例如 IS_ERR()）和调用者采取的适当操作来处理。
&emsp;&emsp;这些函数的调用者应在使用 OPP 后调用 dev_pm_opp_put()。 否则 OPP 的内存将永远不会被释放并导致内存泄漏。 
&emsp;&emsp;dev_pm_opp_find_freq_exact：
&emsp;&emsp;根据**确切**频率和可用性搜索 OPP。 此功能对于启用默认情况下不可用的 OPP 特别有用。
示例：当 SoC 框架检测到可以提供更高频率的情况时，它可以使用此函数在调用 dev_pm_opp_enable 以使其实际可用之前找到 OPP。
```c
...
```
&emsp;&emsp;注意：这是唯一可在不可用的 OPP 上运行的搜索功能。 

&emsp;&emsp;dev_pm_opp_find_freq_floor：搜索**至多**提供频率的可用 OPP。 此功能在搜索较小匹配或按频率递减的顺序对 OPP 信息进行操作时很有用。
&emsp;&emsp;示例：要查找设备的最高 opp： 
```c
freq = ULONG_MAX;
opp = dev_pm_opp_find_freq_floor(dev, &freq);
dev_pm_opp_put(opp);
```
&emsp;&emsp;dev_pm_opp_find_freq_ceil：搜索**至少**提供频率的可用 OPP。 此功能在搜索更高匹配或按频率增加的顺序对 OPP 信息进行操作时很有用。 
&emsp;&emsp;示例 1：要查找设备最低的 opp： 
```c
freq = 0;
opp = dev_pm_opp_find_freq_ceil(dev, &freq);
dev_pm_opp_put(opp);
```
&emsp;&emsp;示例 2：SoC cpufreq_driver->target 的简化实现： 
```c
soc_cpufreq_target(..)
{
...
```
#### 4.4 OPP 可用控制功能 
&emsp;&emsp;向 OPP 库注册的默认 OPP 列表可能无法满足所有可能的情况。 OPP 库提供了一组函数来修改 OPP 列表中某个 OPP 的可用性。 这允许 SoC 框架对哪些 OPP 集在操作上可用进行细粒度的动态控制。 这些功能旨在**暂时**在诸如热考虑因素的情况下移除 OPP（例如，在温度下降之前不要使用 OPPx）。 
&emsp;&emsp;警告：不要在中断上下文中使用这些函数。
&emsp;&emsp;dev_pm_opp_enable：Make a OPP available for operation.
&emsp;&emsp;示例：假设仅当 SoC 温度低于某个阈值时，1GHz OPP 才可用。 SoC 框架实现可能会选择执行以下操作： 
```c
...
```
&emsp;&emsp;dev_pm_opp_disable：Make an OPP to be not available for operation
&emsp;&emsp;示例：假设温度超过阈值，将禁用 1GHz OPP。 SoC 框架实现可能会选择执行以下操作： 
```c
...
```
#### 4.5 OPP 数据检索功能
&emsp;&emsp;由于 OPP 库抽象了 OPP 信息，因此需要一组从 OPP 结构中提取信息的函数。 一旦使用搜索函数检索到 OPP 指针，SoC 框架就可以使用以下函数来检索 OPP 层内部表示的信息。 
&emsp;&emsp;dev_pm_opp_get_voltage：检索由 opp 指针表示的电压。 
&emsp;&emsp;示例：在 cpufreq 转换到不同频率时，SoC 框架需要使用调节器框架将 OPP 表示的电压设置为提供电压的电源管理芯片。 
```c
soc_switch_to_freq_voltage(freq)
{
  /* do things */
...
```
&emsp;&emsp;dev_pm_opp_get_freq：检索由 opp 指针表示的频率。
&emsp;&emsp;示例：假设 SoC 框架使用了几个辅助函数，我们可以传递 opp 指针，而不是执行额外的参数来处理一些数据参数。 
```c
soc_cpufreq_target(..)
{
/* do things.. */
...
```
&emsp;&emsp;dev_pm_opp_get_opp_count：检索设备的可用 opps 数量
&emsp;&emsp;示例：假设 SoC 中的协处理器需要知道表中的可用频率，主处理器可以通知如下： 
```c
...
```
#### 4.6 数据结构
&emsp;&emsp;通常，SoC 包含多个可变的电压域。 每个域由一个设备指针表示。 与 OPP 的关系可以表示如下： 
```shell
SoC
 |- device 1
 ...
 `- device m
```
&emsp;&emsp;OPP 库维护一个内部列表，SoC 框架填充和访问上述各种功能。 然而，代表实际 OPP 和域的结构是 OPP 库本身的内部结构，以允许跨系统重用合适的抽象。 
&emsp;&emsp;struct dev_pm_opp：OPP 库的内部数据结构，用于表示一个 OPP。 除了频率、电压、可用性信息之外，它还包含 OPP 库操作所需的内部簿记信息。 指向该结构的指针被提供给用户，例如 SoC 框架，在与 OPP 层的交互中用作 OPP 的标识符。
&emsp;&emsp;警告：用户不应解析或修改 struct dev_pm_opp 指针。 实例的默认值由 dev_pm_opp_add 填充，但可以通过 dev_pm_opp_enable/disable 函数修改 OPP 的可用性。 
&emsp;&emsp;struct device：这用于向 OPP 层标识域。 设备的性质及其实现留给了 OPP 库（例如 SoC 框架）的用户。 
&emsp;&emsp;总的来说，简单的来看，数据结构的操作表示如下： 
```shell
...
### 5 clk.txt 通用时钟框架
&emsp;&emsp;本文档致力于解释常见的 clk 框架细节，以及如何将平台移植到该框架。这不是一个 include/linux/clk.h 中时钟 api 的详细解释，但也许有一天它会包含该信息。 
#### 5.1 介绍与接口拆分
&emsp;&emsp;通用时钟框架是当今控制各种设备上可用时钟节点的接口。这可能以时钟门控、速率调整、多路复用或其他操作的形式出现。该框架通过 CONFIG_COMMON_CLK 选项启用。 
&emsp;&emsp;接口本身分为两部分，每个部分都与对应部分的细节隔离。首先是struct clk的通用定义，它统一了框架级别的 accounting 和 infrastructure，传统上，这些 accounting 和 infrastructure在各种平台上都是重复的。第二个是 clk.h api 的通用实现，在 drivers/clk/clk.c 中定义。最后是struct clk_ops，其操作由 clk api 实现调用。
&emsp;&emsp;接口的后半部分由使用 struct clk_ops 注册的特定于硬件的回调以及模拟特定时钟所需的相应硬件特定结构组成。 对于本文档的其余部分，对 struct clk_ops 中回调的任何引用，例如 .enable 或 .set_rate，都意味着该代码的硬件特定实现。 同样，对 struct clk_foo 的引用可用作实现假设的“foo”硬件的硬件特定位的便捷简写。 
&emsp;&emsp;将这个接口的两半联系在一起的是 struct clk_hw，它在 struct clk_foo 中定义并在 struct clk_core 中指向。 这允许在公共时钟接口的两个离散部分之间轻松导航。 
#### 5.2 通用数据结构和 api
&emsp;&emsp;下面是来自drivers/clk/clk.c 的通用struct clk_core 定义，为简洁起见进行了修改： 
```c
struct clk_core {
  const char		*name;
...
```
&emsp;&emsp;上述成员构成了时钟树拓扑的核心。 clk api 本身定义了几个面向驱动程序的函数，这些函数在 struct clk 上运行。 该 api 记录在 include/linux/clk.h 中。 
&emsp;&emsp;使用公共结构 clk_core 的平台和设备使用结构 clk_core 中的结构 clk_ops 指针来执行 clk-provider.h 中定义的特定于硬件的操作部分： 
```c
struct clk_ops {
...
```
#### 5.3 硬件时钟实现
&emsp;&emsp;通用 struct clk_core 的优势来自它的 .ops 和 .hw 指针，它们从硬件特定的位中抽象出 struct clk 的细节，反之亦然。 为了说明，请考虑在drivers/clk/clk-gate.c 中的简单门控clk 实现： 
```c
struct clk_gate {
  struct clk_hw	hw;
  void __iomem    *reg;
...
```
&emsp;&emsp;struct clk_gate 包含 struct clk_hw hw 以及有关哪个寄存器和位控制此 clk 门控的特定于硬件的知识。 此处不需要有关时钟拓扑或记帐的任何信息，例如 enable_count 或 notifier_count。 这一切都由通用框架代码和 struct clk_core 处理。
...
### 6 governors.txt
#### 6.1 什么是 CPUFreq Governor
&emsp;&emsp;大多数 cpufreq 驱动程序（实际上，除了 longrun 之外的所有驱动程序），设置大多数 cpu 频率缩放算法都只提供将 CPU 设置为一种频率。为了提供动态频率缩放，cpufreq 内核必须能够告诉这些驱动程序一个“目标频率”。因此，这些特定的驱动程序将被转换为提供`->target/target_index`调用，而不是现有的`->setpolicy`调用。不过，对于“longrun”，一切都保持不变。 
&emsp;&emsp;如何决定应该使用 CPUfreq 策略中的哪个频率？ 这是使用“cpufreq governor”完成的。有两个已经在这个补丁中——它们是已经存在的“powersave”和“performance”，它们分别将频率静态设置为最低或最高频率。 在不久的将来，至少还有两个这样的 governors 将准备添加，但也可能会有更多，因为有各种不同的关于动态频率缩放的理论和模型。使用 cpufreq 为缩放 governors 提供的通用接口，可以对它们进行广泛的测试，并且可以为每种特定用途选择最好的接口。 
&emsp;&emsp;基本上，它是以下流程图： 
```c
...
```
#### 6.2 Governors In the Linux Kernel
##### 6.2.1 Performance
&emsp;&emsp;CPUfreq governor 选项——“performance”将 CPU 静态设置为 scaling_min_freq 和 scaling_max_freq 边界内的最高频率。 
##### 6.2.2 Powersave
&emsp;&emsp;CPUfreq governor 选项——“powersave”将 CPU 静态设置为 scaling_min_freq 和 scaling_max_freq 边界内的最低频率。
##### 6.2.3 Userspace
&emsp;&emsp;CPUfreq governor 选项“userspace”允许用户或任何使用 UID“root”运行的用户空间程序通过在 CPU 设备路径中提供的 sysfs 文件“scaling_setspeed”来将 CPU 设置为特定频率。 
##### 6.2.3 Ondemand
&emsp;&emsp;CPUfreq governor 选项“Ondemand”根据当前使用情况设置 CPU。 为此，CPU 必须能够非常快速地切换频率。有许多 sysfs 文件可访问的参数： 
* sampling_rate：以us（10^-6 秒）为单位测量，这是您希望内核查看 CPU 使用率并决定如何处理频率的间隔。 通常这被设置为大约“10000”或更多的值。 它的默认值是 (cmp. with users-guide.txt)：transition_latency * 1000
请注意，转换延迟以 ns 为单位，而 Sampling_rate 以 us 为单位，因此默认情况下您会获得相同的 sysfs 值。
考虑到转换延迟，应始终调整采样率。 
要将采样率设置为 bash 中转换延迟的 750 倍（如上所述，默认值为 1000），请执行以下操作： 
```shell
echo `$(($(cat cpuinfo_transition_latency) * 750 / 1000)) \
    >ondemand/sampling_rate
```
* sampling_rate_min：
采样率被硬件 transition latency 限制的：`transition_latency * 100`
或被内核限制：
  * 如果设置了 CONFIG_NO_HZ_COMMON，限制是固定的 10 ms.
  * 如果没有设置 CONFIG_NO_HZ_COMMON，或者使用了引导参数 nohz=off，则限制取决于 CONFIG_HZ 选项：
    * HZ=1000: min=20000us  (20ms)
    * HZ=250:  min=80000us  (80ms)
    * HZ=100:  min=200000us (200ms)
  
  内核和硬件延迟限制的最高值被显示并用作最小采样率。 
* up_threshold：定义了 'sampling_rate' 采样之间的平均 CPU 使用率需要多少，以便内核决定是否应该增加频率。 例如，当它设置为其默认值“95”时，意味着在检查间隔之间，CPU 需要平均使用 95% 以上，然后才能决定需要增加 CPU 频率。 
* ignore_nice_load：此参数的值为“0”或“1”。 当设置为“0”（默认值）时，所有进程都计入“cpu utilisation”值。 当设置为“1”时，以“nice”值运行的进程在总体使用情况计算中将不计入（因此被忽略）。 如果您在笔记本电脑上运行 CPU 密集型计算，而您不关心完成需要多长时间，这将非常有用，因为您可以“优化”它并防止它参与决定是否增加 CPU 频率的过程。 
* sampling_down_factor：此参数控制内核在以最高速度运行时决定何时降低频率的速率。 当设置为 1（默认）时，无论当前时钟速度如何，都会在相同的时间间隔内做出重新评估负载的决定。 但是当设置为大于 1（例如 100）时，它充当调度间隔的乘数，用于在 CPU 由于高负载而处于最高速度时重新评估负载。 这通过减少负载评估的开销并帮助 CPU 在真正繁忙时保持其最高速度而不是来回切换速度来提高性能。 此可调参数对较低速度/较低 CPU 负载下的行为没有影响。 
* powersave_bias：此参数采用 0 到 1000 之间的值。它定义了将从目标中剔除的目标频率的百分比（乘以 10）值。 例如，当设置为 100 -- 10% 时，当按需调节器的目标是 1000 MHz 时，它将以 1000 MHz - (1000 MHz 的 10%) = 900 MHz 为目标。 默认设置为 0（禁用）。 
...
#### 1.1 iio_configfs.rst
工业IIO配置支持
##### 1.1.1 概述
&emsp;&emsp;Configfs是一个基于文件系统的内核对象管理器。IIO 使用一些可以使用 configfs 轻松配置的对象（例如：设备、触发器）。
&emsp;&emsp;有关 configfs 如何工作的更多信息，请参阅`Documentation/filess/configfs.rst`。
##### 1.1.2 用法
&emsp;&emsp;为了在 IIO 中使用 configfs 支持，我们需要在编译时通过 CONFIG_IIO_configfs 配置选项选择它。
&emsp;&emsp;然后，装载 configfs 文件系统（通常在/config目录下）：
```shell
$ mkdir /config
$ mount -t configfs none /config
```
&emsp;&emsp;此时，所有默认的 IIO 组都将被创建，并且可以在 /config/IIO 下访问。接下来的章节将描述可用的 IIO 配置对象。

##### 1.1.3 软件触发
&emsp;&emsp;IIO 默认的 configfs 组之一是“触发器”组。当 mounted configfs 时，它可以自动访问，并且可以在 /config/iio/triggers 下找到。
&emsp;&emsp;IIO 软件触发器实现提供了对创建多个触发器类型的支持。一个新的触发器类型通常被实现为一个独立的内核模块，遵循 include/linux/iio/sw_trigger.h 中的接口：
```c
/*
...
```
&emsp;&emsp;每种触发器类型在 /config/iio/triggers 下都有自己的目录。加载 iio-trig-sample 模块将创建“trig-sample”触发器类型目录 /config/iio/tricks/trig-sample。
&emsp;&emsp;我们支持以下中断源（触发器类型）：
* hrtimer，使用高分辨率定时器作为中断源

###### 1.1.3.1 Hrtimer 触发器的创建和销毁
&emsp;&emsp;加载 iio-trig-hrtimer 模块将注册 hrtimer 触发器类型，允许用户在 /config/iio/triggers/hrtimer 下创建 hrtimer 触发。
&emsp;&emsp;例如：
```shell
$ mkdir /config/iio/triggers/hrtimer/instance1
$ rmdir /config/iio/triggers/hrtimer/instance1
```
&emsp;&emsp;每个触发器可以具有一个或多个特定于触发器类型的属性。

###### 1.1.3.1 Hrtimer 触发器的类型属性
“hrtimer”触发器类型在 /config 目录中没有任何可配置的属性。它将 sampling_frequency 属性引入触发器目录。


#### 1.2 adc.txt
通用 ADCs 属性
&emsp;&emsp;子节点可操作的属性：
* bipolar：Boolean 类型，如果设置了，那么通道工作在 bipolar 模式下。
* diff-channels：此ADC的差分通道多路复用。第一个值指定正电压输入引脚，第二个值指定负电压输入引脚。

&emsp;&emsp;注释：同时接受正电压和负电压输入的 ADC 成为 biploar（双极型）ADC。而仅接受正电压输入的 ADC 成为单极 ADC。
&emsp;&emsp;示例：
```shell
Example:
	adc@0 {
