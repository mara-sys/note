...
* `desc_to_gpio(const struct gpio_desc *desc)`：根据输入的gpio_desc返回对  的
...
&emsp;&emsp;当前 Linux 内核的设计使用一个大的数字空间来为每个单独的 IRQ 源被分配一个不同的数字。当只有一个中断控制器时，这很简单，但是在具有多个中断控制器的系统中，内核必须确保每个控制器都被分配到不重叠的 Linux IRQ 编号分配。  
