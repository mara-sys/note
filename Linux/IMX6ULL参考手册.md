...
#### 15.5.9 APBH DMA 通道 n 命令寄存器 (APBH_CHn_CMD) 
&emsp;&emsp;APBH DMA 通道 n 命令寄存器指定要为当前命令链项执行的 DMA 事务。
&emsp;&emsp;命令寄存器控制该通道的每个 DMA 命令的整体操作。 它包括传输到设备或从设备传输的字节数、包含在此命令结构中的 APB PIO 命令字的数量、是否在命令完成时中断、是否将附加命令链接到该命令的末尾以及此传输是否 是读或写 DMA 传输。 
